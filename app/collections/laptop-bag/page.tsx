...
export default function LaptopBagCollectionPage() {
  const [sortBy, setSortBy] = useState<string>("featured");
  const [laptopBagProducts, setLaptopBagProducts] = useState<Product[]>([]);
    ...
    async function fetchProducts() {
      try {
        ...
        if (response.ok && data.products) {
          // Filter for laptop bags (case-insensitive)
          const laptopBags = data.products.filter((p: Product) => 
            p.category?.toLowerCase().includes("laptop")
          );
          setLaptopBagProducts(laptopBags);
        } else {
          setError(data.error || 'Failed to load products');
          setLaptopBagProducts([]);
        }
      } catch (err) {
  ...
  const sortedProducts = [...laptopBagProducts].sort((a, b) => {
    switch (sortBy) {
      case "price-low":
      ...
      <div className="container mx-auto px-4 md:px-6 lg:px-8 py-8 md:py-12">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-sm md:text-base text-gray-600 max-w-2xl mx-auto">
            Professional laptop bags designed for style and protection. Perfect for work and travel.
          </p>
        </div>
        ...
        <div className="flex justify-between items-center mb-8 pb-6 border-b border-gray-200">
          <p className="text-sm text-gray-600">
            {laptopBagProducts.length} {laptopBagProducts.length === 1 ? "Product" : "Products"}
          </p>
          <div className="flex items-center gap-2">
