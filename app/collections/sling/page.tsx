    ...
    async function fetchProducts() {
      try {
        ...
        if (response.ok && data.products) {
          const slings = data.products.filter((p: Product) => p.category?.toLowerCase().includes('sling'));
          setProducts(slings);
        }
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="container mx-auto px-4 md:px-6 lg:px-8 py-12">
        <h1 className="text-4xl md:text-5xl font-serif tracking-[0.15em] text-center mb-4">SLING BAGS</h1>
        <p className="text-gray-600 text-center max-w-2xl mx-auto mb-12">Compact and stylish sling bags for on-the-go elegance</p>
        {loading ? (
