...
export default function BackpackCollectionPage() {
  const [sortBy, setSortBy] = useState<string>("featured");
  const [backpackProducts, setBackpackProducts] = useState<Product[]>([]);
    ...
    async function fetchProducts() {
      try {
        ...
        if (response.ok && data.products) {
          // Filter for backpacks (case-insensitive)
          const backpacks = data.products.filter((p: Product) => 
            p.category?.toLowerCase() === "backpack"
          );
          setBackpackProducts(backpacks);
        } else {
  ...
  const sortedProducts = [...backpackProducts].sort((a, b) => {
    switch (sortBy) {
      case "price-low":
      ...
      <div className="container mx-auto px-4 md:px-6 lg:px-8 py-8 md:py-12">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-sm md:text-base text-gray-600 max-w-2xl mx-auto">
            Versatile backpacks for every adventure. Combining style, comfort, and functionality.
          </p>
        </div>
        ...
        <div className="flex justify-between items-center mb-8 pb-6 border-b border-gray-200">
          <p className="text-sm text-gray-600">
            {backpackProducts.length} {backpackProducts.length === 1 ? "Product" : "Products"}
          </p>
          <div className="flex items-center gap-2">
